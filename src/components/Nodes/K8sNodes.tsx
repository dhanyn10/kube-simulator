...
export const BaseNode = memo(({ children, data, selected, title, icon: Icon, color, colorHex }: { 
  ...
  return (
    <div className={cn(
      "group relative min-w-[160px] bg-slate-800 border-2 rounded-lg p-3 transition-all duration-200",
      ...
      <div className="space-y-1.5">
        <div className="text-xs font-mono font-bold text-slate-100 truncate">{data.label}</div>
        {data.image ? (
...
export const ServiceNode = memo((props: NodeProps) => {
  ...
  return (
    <div className={cn(
      "relative min-w-[180px] p-4 bg-slate-900 border-2 border-amber-500 rounded-lg shadow-2xl transition-all",
      ...
      <div className="text-xs font-bold text-slate-100 mb-1">{data.label}</div>
...
export const DeploymentNode = memo((props: NodeProps) => {
  ...
  return (
    <div className={cn(
      "relative min-w-[320px] min-h-[160px] bg-violet-600/5 border-2 border-dashed rounded-xl p-6 transition-all duration-200",
      props.selected ? "border-violet-500 ring-4 ring-violet-500/10" : "border-slate-800 hover:border-slate-700",
    )}>
      <div className="absolute -top-3 left-6 flex items-center gap-2">
        <span className="text-[9px] px-2 py-0.5 bg-violet-600 rounded-sm font-bold tracking-tighter text-white uppercase">
          DEPLOYMENT
        </span>
        <span className="text-xs font-mono font-bold text-violet-300 tracking-tight">
          {data.label}
        </span>
      </div>
