...
export const resizeHandlers = (set: any, get?: () => FlowState) => ({
  ...
  onNodeResizeStop: (_event: any, node: Node) => {
    if (get && node) {
      ...
      get().addActivityLog?.(`[Canvas Action] Resized card '${label}' (${currentNode.type}) to size: ${w}x${h}px at coordinates (x1:${x1}, y1:${y1}, x2:${x2}, y2:${y2}) [Top-Left: (${x1}, ${y1}), Bottom-Right: (${x2}, ${y2})]`);
    }
    set({
