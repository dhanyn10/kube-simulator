...
import { Settings, Plus, Trash2, Key, Type } from 'lucide-react';
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-[9px] font-bold text-slate-500 uppercase flex items-center gap-1.5">
            <Settings size={10} /> Data (Key-Value)
          </label>
          <button
            onClick={addData}
            className="flex items-center gap-1 px-2 py-1 rounded bg-teal-500 hover:bg-teal-600 text-white text-[9px] font-bold transition-all"
          >
        ...
        <div className="space-y-2">
          {configData.length === 0 && (
            <div className={cn(
              ...
              <Settings size={20} className="mx-auto mb-1.5 opacity-20" />
              <p className="text-[9px]">Belum ada data konfigurasi</p>
            </div>
          )}
          ...
          {configData.map((item: any, idx: number) => (
            <div key={idx} className={cn(
                ...
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className={cn(
                      ...
                      colorMode === 'dark' ? "bg-slate-800 border-slate-700 text-slate-200 focus:border-teal-500/50" : "bg-slate-50 border-slate-200 text-slate-800 focus:border-teal-400"
                    )}
                  />
                </div>
                <button
              ...
              <div className="relative">
                <Type size={10} className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-500" />
                <input
                  type="text"
                  placeholder="Value"
                  value={item.value}
