...
const getItemClassName = (isCurrentStep: boolean, colorMode: 'dark' | 'light'): string => {
  if (isCurrentStep) {
    if (colorMode === 'dark') {
      return 'bg-violet-950/40 border-violet-500 text-violet-300';
    }
    return 'bg-violet-50 border-violet-500 text-violet-900';
  }
  if (colorMode === 'dark') {
    return 'border-transparent hover:bg-slate-800/50 text-slate-300';
  }
  return 'border-transparent hover:bg-violet-50/50 text-slate-700';
};
...
export function HistoryPanel({ colorMode }: Readonly<HistoryPanelProps>) {
  ...
  const renderContent = () => {
    if (isLoading && historyLogs.length === 0) {
      return (
    ...
    if (historyLogs.length === 0) {
      return <div className="p-8 text-center text-[10px] opacity-50 italic">No activity recorded</div>;
    }
