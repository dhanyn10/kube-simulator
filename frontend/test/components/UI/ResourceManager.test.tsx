...
describe('ResourceManager', () => {
  ...
  it('handles overwriting an existing project with confirmation', async () => {
    const project = { id: 1, name: 'Project 1', content: JSON.stringify({ nodes: [{ id: 'n1' }], edges: [] }), updatedAt: Date.now() / 1000 };
    mockGetProjects.mockResolvedValue([project]);

    useFlowStore.setState({
      nodes: [{ id: 'n2', type: 'Pod', position: { x: 0, y: 0 }, data: {} } as any],
    });
    ...
    // Click Overwrite again and click YES to confirm
    fireEvent.click(screen.getByText('Overwrite'));
    await act(async () => {
  ...
  it('filters sidebar tabs with search input', async () => {
    await act(async () => {
      render(<ResourceManager isOpen={true} onClose={() => {}} />);
    });
    ...
    fireEvent.change(searchInput, { target: { value: 'nonexistent-tab' } });
