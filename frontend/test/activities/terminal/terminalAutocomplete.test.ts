...
describe('terminalAutocomplete', () => {
  ...
  it('returns next keyword suggestions sequentially with context labels for kubectl scale sequence', () => {
    ...
    expect(depSugg.some(s => s.value === 'kubectl scale deployment/web-deployment ')).toBe(true);
    ...
    expect(flagSugg[0].value).toBe('kubectl scale deployment/web-deployment --replicas=');
    ...
    expect(numSugg.some(s => s.value === 'kubectl scale deployment/web-deployment --replicas=3')).toBe(true);
  });
  ...
  it('handles autocomplete for rollout, set image, and describe commands with context labels', () => {
    const mockNodes: Node[] = [
    ...
    expect(rolloutSuggestions.some(s => s.label === 'deploy/api-service')).toBe(true);
    expect(rolloutSuggestions.some(s => s.value === 'kubectl rollout status deploy/api-service')).toBe(true);
    ...
    expect(describeSuggestions.some(s => s.value === 'kubectl describe deploy api-service')).toBe(true);
  });
});
